...
One-command benchmark runner for Scloud+ KEM end-to-end performance.
...
Important PRG note:
  This repository's KEM implementation mixes PRGs by design/code path: AES-128-CTR
  is used for public matrix expansion, while SHAKE256 is used for seed expansion
  and secret/noise sampling. This script benchmarks the whole mixed KEM for ref
  and optimized builds; it does not pretend to produce separate all-AES-KEM and
  all-SHAKE-KEM numbers because those variants do not exist in this checkout.
...

{
  ...
  echo "- KEM PRG model in this checkout: mixed, not selectable all-AES or all-SHAKE."
  echo "- Public matrix expansion uses AES-128-CTR via AES128_load_schedule/AES128_CTR_enc_sch."
  echo "- Seed expansion and secret/noise sampling use SHAKE256 via scloudplus_F, shake256, and incremental SHAKE256 squeeze APIs."
  echo "- Fresh entropy uses randombytes; this checkout maps it to OpenSSL RAND_bytes when USE_OPENSSL is defined."
  echo "- To benchmark an all-AES-KEM or all-SHAKE-KEM end to end, separate implementation variants must be added first."
  echo
  echo "## PRG usage scan"
  if rg -n "AES128_CTR_enc_sch|AES128_load_schedule|scloudplus_F|shake256|shake256_absorb_once|shake256_squeezeblocks|randombytes|RAND_bytes" "$ROOT_DIR/src" >/tmp/scloudplus_prg_scan.$$ 2>/dev/null; then
...
for i in "${!VARIANT_NAMES[@]}"; do
...

  {
    ...
    echo "CFLAGS: $flags"
    echo "KEM PRG model: mixed AES-128-CTR matrix expansion + SHAKE256 seed/noise sampling"
